...
/// <summary>
/// Stepper button that increments the number field value. Supports press-and-hold
/// repeat via JS pointer event listeners.
/// </summary>
public class NumberFieldIncrement : BlazeElement<NumberFieldButtonState>
{
    [CascadingParameter]
    ...
    private bool IsEffectivelyDisabled =>
        ...
        (Context.Max.HasValue && Context.Value.HasValue && Context.Value.Value >= Context.Max.Value);

    // Guard against registering the press-and-hold JS listener more than once.
    // The Root loads JS asynchronously and then re-renders, so this component's
    // OnAfterRenderAsync will fire a second time with a non-null JsModule.
    ...
    protected override void BuildRenderTree(RenderTreeBuilder builder)
    {
        ...
        builder.AddAttribute(6, "aria-label", "Increase");
